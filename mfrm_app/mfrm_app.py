"""
MFRMSight — 多面 Rasch 模型分析工具 v0.2
=========================================

快速使用:
  python mfrm_app.py data.txt                 # 终端输出
  python mfrm_app.py data.txt -o out.xlsx     # 导出 Excel
  python mfrm_app.py data.txt -o out.docx     # 导出 Word
  python mfrm_app.py data.xlsx               # 从 Excel 加载

Python API:
  from mfrm_app import run
  engine = run("data.txt")
  engine.to_excel("output.xlsx")
  engine.to_word("output.docx")
...
算法:
  Andrich Rating Scale Model (1978)
  Fisher-scoring JMLE, 分阶段 PROX → JMLE
  Ridge 衰减 + 步长截断 + 最佳参数追踪
...
def parse_facets_txt(filepath: str) -> dict:
    ...
    return _build_data(raw, n_facets, [l for _, l in infos[0]], [l for _, l in infos[1]], [l for _, l in infos[2]], [l for _, l in infos[3]])
...
def parse_excel(filepath: str) -> dict:
    ...
    # 找表头行（含"编号"/"评分人"）
    header_row = 1  # 默认第 2 行
    for i in range(len(df_raw)):
        s = [str(v) for v in df_raw.iloc[i] if pd.notna(v)]
        if any("编号" in x or "评分人" in x for x in s):
            header_row = i; break
    col_names = [str(df_raw.iloc[header_row, j]) if pd.notna(df_raw.iloc[header_row, j]) else f"Col{j}" for j in range(df_raw.shape[1])]
    # 解析列名 → 学生和标准
    s_set, c_set = set(), set()
    ...
    for ri in range(header_row + 1, len(df_raw)):
        ...
        n_r += 1
        rater_labels = [str(df_raw.iloc[ri, 1]) for ri in range(header_row + 1, len(df_raw)) if pd.notna(df_raw.iloc[ri, 0])]
    raw = np.array(data_rows)
    return _build_data(raw, 3, s_list, rater_labels if rater_labels else [f"R{i}" for i in range(1, n_r + 1)], c_list, ["Item1"])


def _extract_num(s): return int(re.findall(r'\d+', str(s))[-1]) if re.findall(r'\d+', str(s)) else 1


def _build_data(raw, n_facets, s_labels, r_labels, c_labels, i_labels):
    return {
        ...
        "s_labels": s_labels or [f"S{i+1}" for i in range(int(raw[:, 0].max()))],
        "r_labels": r_labels or [f"R{i+1}" for i in range(int(raw[:, 1].max()))],
        "c_labels": c_labels or [f"C{i+1}" for i in range(int(raw[:, 2].max()))],
        "i_labels": i_labels or [f"I{i+1}" for i in range(max(1, int(raw[:, 3].max()) if n_facets >= 4 and raw.shape[1] > 3 else 1))],
    }
...
class Engine:
    ...
    def _probs(self):
        intc = (self.theta[self.s_idx] - self.delta[self.r_idx] - self.alpha[self.c_idx] - self.beta[self.i_idx])
    ...
    def _facet(self, obs, param, labels, n):
        ...
        for p in range(n):
            ...
            rows.append({"label": labels[p], "total": total, "obs_avg": self.scores[idx].mean(), "fair_avg": self.exp_scores[idx].mean(), "meas": param[p], "se": se, "infit": infit, "outfit": outfit})
    ...
    def report(self):
        ...
        r = {"summary": {"N": self.N, "n_s": self.n_s, "n_r": self.n_r, "n_c": self.n_c, "n_i": self.n_i, "score_range": f"{self.min_s}-{self.max_s}", "obs_mean": float(self.scores.mean()), "exp_mean": float(self.exp_scores.mean()), "resid_sd": float(self.resid.std(ddof=1)), "stres_sd": float(self.z.std(ddof=1)), "var_exp": round(self.var_exp, 2), "ll": round(self.ll_final, 2)}, "facets": {}}
    ...
    def print(self):
        ...
        print(f"\n{'='*72}\nMFRMSight — 多面 Rasch 模型分析报告\n{'='*72}")
        print(f"{s['N']} 条反应 | {s['n_s']} 学生 × {s['n_r']} 评分者 × {s['n_c']} 标准 × {s['n_i']} 题目")
        print(f"分数范围: {s['score_range']} | 方差解释: {s['var_exp']}% | Log-likelihood: {s['ll']:.0f}")
        print(f"Obs Mean  = {s['obs_mean']:.3f}   Exp Mean  = {s['exp_mean']:.3f}")
        print(f"Resid SD  = {s['resid_sd']:.4f}   StRes SD  = {s['stres_sd']:.4f}")
        for fn, fd in r["facets"].items():
            if not fd["rows"]: continue
            sep, rel = fd["separation"], fd["reliability"]
            print(f"\n{'─'*72}\n{fn}  —  Sep = {sep:.2f}  |  Rel = {rel:.3f}\n{'─'*72}")
    ...
    def to_excel(self, path):
        import pandas as pd
        r = self.report()
        dfs = {n: pd.DataFrame(f["rows"]) for n, f in r["facets"].items()}
    ...
    def to_word(self, path):
        ...
        doc.save(path); print(f"[OK] Word: {path}")
...
def run(input_path: str, output: Optional[str] = None, quiet: bool = False) -> Engine:
    p = Path(input_path)
    d = parse_facets_txt(input_path) if p.suffix == ".txt" else parse_excel(input_path)
    eng = Engine(d).fit()
    if not quiet: eng.print()
    if output:
        o = Path(output)
        if o.suffix == ".xlsx": eng.to_excel(output)
        elif o.suffix == ".docx": eng.to_word(output)
        else: eng.to_excel(output)
    return eng


def main():
    ap = argparse.ArgumentParser(description="MFRMSight v0.2 — 多面 Rasch 模型分析", epilog="示例: python mfrm_app.py data.txt -o out.xlsx")
    ap.add_argument("input", help="输入文件 (.txt 或 .xlsx)")
    ap.add_argument("-o", "--output", help="输出文件 (.xlsx 或 .docx)")
    ap.add_argument("-q", "--quiet", action="store_true")
    a = ap.parse_args()
    try: run(a.input, a.output, a.quiet)
